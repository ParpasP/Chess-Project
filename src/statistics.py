...
def openings(games_df, type=None, limit=20):
    """Plot the best-performing openings as White and as Black.

    Parameters
    ----------
    games_df : pd.DataFrame
        Games dataframe with 'time_class', 'my_color', 'simple_opening',
        and 'result' columns.
    type : str, optional
        If given, restrict to this time_class (e.g. "blitz").
    ...
    df["result"] = df["result"].map(score_map)
...
def daytime(games_df, type=None):
    """Plot number of games by hour of day, split by result.

    Parameters
    ----------
    games_df : pd.DataFrame
        Games dataframe with 'Time' (datetime or datetime-parseable) and
        'result' columns.
    type : str, optional
        If given, restrict to this time_class.
    """
    df = games_df.copy()
    if type is not None:
        df = df[df["time_class"] == type]
